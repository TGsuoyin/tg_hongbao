...
class LuckyReport extends Repository
{
    ...
    {
        $obj = \App\Models\LuckyMoney::query()->where('created_at', '>', Carbon::parse($start_date))
            ->where('created_at', '<', Carbon::parse($end_date))
            ->groupBy('date')
            ->orderBy('date','desc');
        if($group_id){
            $obj = $obj->where('chat_id',$group_id);
        }
            $obj = $obj->get([DB::raw('DATE(created_at) as date'), DB::raw('COUNT(*) as value')])
            ->toArray();
            return  $obj;
    }
