...
class LuckyAmount extends Line
{
    protected $label = '发包金额';
    // 保存自定义参数
    protected $data = [];
    ...
    public function getByDate($start_date,$end_date,$tgId){
        return LuckyMoney::query()->where('created_at','>',Carbon::parse($start_date))
            ->where('created_at','<',Carbon::parse($end_date))
            ->where('sender_id',$tgId)
            ->groupBy('date')
            ->get([DB::raw('DATE(created_at) as date'),DB::raw('SUM(received) as value')])
            ->toArray();
    }
