...
class LuckyHistoryThunderAmount extends Line
{
    protected $label = '抢包中雷损失';
    // 保存自定义参数
    protected $data = [];
    ...
    public function getByDate($start_date,$end_date,$tgId){
        return \App\Models\LuckyHistory::query()->where('created_at','>',Carbon::parse($start_date))
            ->where('created_at','<',Carbon::parse($end_date))
            ->where('user_id',$tgId)
            ->where('is_thunder',1)
            ->groupBy('date')
            ->get([DB::raw('DATE(created_at) as date'),DB::raw('SUM(lose_money) as value')])
            ->toArray();
    }
