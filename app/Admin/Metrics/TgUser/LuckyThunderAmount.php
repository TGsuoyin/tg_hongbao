...
class LuckyThunderAmount extends Line
{
    protected $label = '发包中雷盈利';
    // 保存自定义参数
    protected $data = [];
    ...
    public function getByDate($start_date,$end_date,$tgId){
        return \App\Models\LuckyHistory::query()->where('lucky_history.created_at','>',Carbon::parse($start_date))
            ->where('lucky_history.created_at','<',Carbon::parse($end_date))
            ->leftJoin('lucky_money','lucky_money.id','=','lucky_history.lucky_id')
            ->where('lucky_money.sender_id',$tgId)
            ->where('lucky_history.is_thunder',1)
            ->groupBy('date')
            ->get([DB::raw('DATE(lucky_history.created_at) as date'),DB::raw('SUM(lucky_history.lose_money) as value')])
            ->toArray();
    }
